...
def _get_pinned_thread_ids_for_board(board_id: str) -> set[str]:
    """Resolve persisted pin keys into thread ids for one board."""
    prefix = f"{board_id}:"
    return {
        key[len(prefix):]
        for key in get_settings().get_pinned_thread_keys()
...
async def enforce_global_quota() -> PruneResult:
    ...
    all_threads = await get_all_active_threads_global(
        pinned_thread_keys=settings.get_pinned_thread_keys(),
    )
...
async def prune_board(board_id: str, now: Optional[int] = None) -> PruneResult:
    ...
    try:
        ...
        pinned_thread_ids = _get_pinned_thread_ids_for_board(board_id)

        # -----------------------------------------------------------------
        # Step 1: Mark expired threads.
        # Threads expire when the OP row's expiry_timestamp is at or before now.
        # -----------------------------------------------------------------
        abandoned_ids = await mark_expired_threads(
            db,
            now=now,
        ...
        # -----------------------------------------------------------------
        capped_threads = await enforce_thread_cap(
            db,
            max_threads,
            pinned_thread_ids=pinned_thread_ids,
        )
