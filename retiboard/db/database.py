...
async def mark_expired_threads(
    ...
    async with db.execute(
        ...
        rows = await cur.fetchall()
        expired_ids = [row["thread_id"] for row in rows]
...
async def get_all_active_threads_global() -> list[tuple[str, str, list[str], int, int]]:
    """
...
async def enforce_thread_cap(
    ...
    """
    count = await get_thread_count(db)
    if count <= max_threads:
        return []

    excess = count - max_threads

    # Find the oldest active threads to cull.
    async with db.execute(
        """
        SELECT thread_id FROM posts
        WHERE post_id = thread_id AND is_abandoned = 0
        ...
        rows = await cur.fetchall()
        cull_ids = [row["thread_id"] for row in rows]
