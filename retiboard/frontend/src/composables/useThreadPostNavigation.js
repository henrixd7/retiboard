...
export function getScrollEdgeDirection(delta, currentDirection = 'down') {
  if (Math.abs(delta) <= 8) return currentDirection
...
export function useThreadPostNavigation(options) {
  ...
  function onWindowScroll() {
    ...
    const nextDirection = getScrollEdgeDirection(delta, scrollEdgeDir.value)
    lastScrollY = nextScrollY
    scrollEdgeDir.value = nextDirection
  }
